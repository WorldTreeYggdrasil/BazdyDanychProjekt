...
def Zarzadz_produktem(c):
    ...
    0.Powrot"""
        ...
        # Print Data
            ...
            try:
                c.execute(
                    "SELECT * FROM Produkty,Pracownicy WHERE kod_kreskowy = "
                    + str(inp)
                    + " AND Pracownicy.ID_pracownika = Produkty.ID_Pracownika;"
                )
                x = list(c.fetchone())
            ...
            try:
                c.execute(
                    "SELECT * FROM Produkty,Pracownicy WHERE ID_produktu = "
                    + str(inp)
                    + " AND Pracownicy.ID_pracownika = Produkty.ID_Pracownika;"
                )
        ...
        # Delete Data
            ...
            c.execute(
                "SELECT ID_produktu, kod_kreskowy,Nazwa FROM Produkty WHERE Nazwa IS NULL"
            )
            for x in c.fetchall():
            ...
            c.execute(
                "SELECT ID_produktu, kod_kreskowy,Opis FROM Produkty WHERE Opis IS NULL"
            )
            for x in c.fetchall():
                print("ID:" + str(x[0]) + " Kod:" + str(x[1]))
            input("Wcisnij [ENTER] aby kontynuowac")
            ...
            try:
                c.executemany(
                    "INSERT INTO Produkty (Nazwa,ilosc,opis,kod_kreskowy,ID_pracownika,Data_pomiaru) VALUES(%s, %s, %s,%s,0,%s)",
                    importdata,
                )
                ...
                if "duplicate key" in str(e):
                    ...
                    c.execute("ROLLBACK")
                    c.executemany(
                        "UPDATE Produkty SET Nazwa = %s,ilosc = %s, opis = %s, kod_kreskowy = %s, ID_pracownika = 0, Data_pomiaru = %s WHERE kod_kreskowy = %s",
                        importdata,
                    )
            ...
            try:
                c.execute(
                    "SELECT * FROM Produkty,Pracownicy WHERE Produkty.ID_Pracownika = Pracownicy.ID_Pracownika"
                )
                for x in c.fetchall():
                    print(x)
            except Exception as e:
...
def Zarzadz_pracownikiem(c):
    ...
    0.Powrot"""
        ...
        if inp == str(0):
            ...
            try:
                c.execute(
                    "SELECT * FROM Pracownicy WHERE ID_pracownika =" + str(inp) + ";"
                )
            ...
            try:
                ...
                c.execute(
                    "SELECT * FROM Pracownicy WHERE nazwisko ='" + str(inp) + "';"
                )
            ...
            try:
                ...
                c.executemany(
                    "UPDATE Pracownicy SET Is_zatrudniony = 0 WHERE ID_pracownika = %s;",
                    importdata,
                )
                if c.rowcount < 1:
            ...
            try:
                c.executemany(
                    "INSERT INTO Pracownicy (Imie,Nazwisko,Data_zatrudnienia,Is_zatrudniony) VALUES(%s, %s, %s,1)",
                    importdata,
                )
